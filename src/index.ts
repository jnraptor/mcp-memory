...
app.delete("/:userId/memories/:memoryId", async (c) => {
	...
	try {
		...
		try {
			...
			console.log(
				`Attempted to delete vector ${memoryId} for user ${userId} from Vectorize.`,
			);
		} catch (vectorError) {
			console.error(
				`Failed to delete vector ${memoryId} for user ${userId} from Vectorize:`,
				vectorError,
			);
		}
...
app.put("/:userId/memories/:memoryId", async (c) => {
	...
	try {
		...
		if (!body || typeof body.content !== "string" || body.content.trim() === "") {
			return c.json(
				{ success: false, error: "Invalid or missing content in request body" },
				400,
			);
		}
	...
	try {
		...
		try {
			...
			console.error(
				`Failed to update vector ${memoryId} for user ${userId} in Vectorize:`,
				vectorError,
			);
		}
