...
export async function storeMemoryInD1(
	...
	try {
		const stmt = env.DB.prepare("INSERT INTO memories (id, userId, content) VALUES (?, ?, ?)");
 ...
 */
export async function deleteMemoryFromD1(
	memoryId: string,
	userId: string,
	env: Env,
): Promise<void> {
	try {
		await env.DB.prepare("DELETE FROM memories WHERE id = ? AND userId = ?")
...
export async function updateMemoryInD1(
	...
	try {
		const stmt = env.DB.prepare("UPDATE memories SET content = ? WHERE id = ? AND userId = ?");
		const result = await stmt.bind(newContent, memoryId, userId).run();

		// Check the meta property for changes
		if (!result.meta || result.meta.changes === 0) {
			throw new Error(
				`Memory with ID ${memoryId} not found for user ${userId} or content unchanged.`,
			);
		}
